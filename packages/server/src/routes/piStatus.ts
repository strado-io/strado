...
export async function registerPiStatusRoutes(app: FastifyInstance) {
  app.post('/api/pi/status', async (req) => {
    ...
    if (status !== 'closed') void app.deps.turnDiary.refresh('pi', cwd, sessionId ?? '1', status);
    ...
    if (status === 'closed') app.deps.piStatus.remove(cwd, sessionId ?? '1');
    else app.deps.piStatus.set(cwd, status, sessionId ?? '1');
    ...
    const key = sessionKeyFor('pi', cwd, sessionId ?? '1');
