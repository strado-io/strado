// preview_* tools: per-worktree browser context for agents (moved from packages/desktop/preview-mcp.cjs).
//
...
export function previewTools(env = process.env) {
  const SERVER = env.STRADO_SERVER || 'http://127.0.0.1:7777';
  const WORKTREE = env.STRADO_WORKTREE || process.cwd();
  const sandboxed = Boolean(env.STRADO_SERVER_SOCKET);
  ...
  class Cdp {
    ...
    connect() {
      return new Promise((resolve, reject) => {
        ...
        if (!WS) { reject(new Error('this Node has no global WebSocket; Node 22 or newer is required')); return; }
  ...
  // Inside a sandbox there is no route to the host's loopback or CDP; say so
  // instead of hanging on a connect that can never succeed.
  if (!sandboxed) return TOOLS;
  return TOOLS.map((t) => ({ ...t, run: async () => text('preview tools are not available inside a sandbox') }));
}
