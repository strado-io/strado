...
describe('useColumnWidths', () => {
  it('uses minmax for the branch column in the template', () => {
    ...
    expect(result.current.gridTemplate).toBe('110px 90px 130px minmax(300px, 1fr) 96px 110px 88px');
  });

  it('exposes totalWidth as the sum of all column widths', () => {
    const { result } = renderHook(() => useColumnWidths());
    expect(result.current.totalWidth).toBe(110 + 130 + 300 + 96 + 110 + 90 + 88); // 924
  });
  ...
  it('ignores stored widths for columns that no longer exist', () => {
    ...
    const { result } = renderHook(() => useColumnWidths());
    expect(result.current.gridTemplate).toBe('150px 90px 130px minmax(300px, 1fr) 96px 110px 88px');
  });
});
